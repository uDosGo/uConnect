  ...
  >
    ...
    <header class="proseui-header">
      <div class="proseui-header-left">
        ...
        <span class="text-xs" :style="{ color: 'var(--usx-color-on-surface-variant)' }">UDOUI Document Surface</span>
      </div>
      <div class="proseui-header-controls">
        <!-- Palette dot -->
        <span
          class="palette-dot"
          :style="{ background: store.paletteDotColors[store.palette].bg, borderColor: store.paletteDotColors[store.palette].accent }"
          @click="cyclePalette"
...
npm run dev</code></pre>
        ...
        <div v-if="store.activeView === 'story'" class="story-wrapper">
          <div class="prose-view">
            <h1>Story Wizard</h1>
            <p style="color: var(--usx-color-on-surface-variant); margin-bottom: 1rem;">Create step-by-step guides and walkthroughs.</p>

...
export async function main(argv: string[]): Promise<void> {
  ...
  // Register ghost commands
  const { registerGhostCommands } = await import('./actions/ghost.js');
  registerGhostCommands(program);
  
  // Register setup command
  const { registerSetupCommand } = await import('./actions/setup.js');
  registerSetupCommand(program);
  
  // Register contacts commands
  const { registerContactsCommands } = await import('./actions/contacts.js');

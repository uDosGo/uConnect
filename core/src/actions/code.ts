...
import { callAgent } from '../services/mastra-agent.js';
...
/**
 * Generate code using Mastra agents
 */
async function generateCode(prompt: string): Promise<string> {
  // Try Mastra first
  const result = await callAgent('codegen', prompt);
  
  // If mock response and we have API key, it will be real
  // If no API key, it returns mock (graceful fallback)
  return result;
}

/**
 * Reason about complex problems using DSC2
 */
export async function codeReason(options: { prompt: string; steps?: boolean }) {
  console.log(`🧠 DSC2 reasoning: ${options.prompt.substring(0, 100)}...`);
  
  const result = await callAgent('planner', options.prompt);
  
  if (options.steps) {
    // Parse and display steps nicely
    console.log('\n📋 Reasoning Steps:\n');
    // Format output
  }
  ...
  // Option to copy to clipboard (commented out for now due to TypeScript issues)
...
export function registerCodeCommands(program: Command) {
  const code = program.command('code').description('Mastra-powered code generation and analysis');
  ...
  code
    .command('reason')
    .argument('<prompt>', 'Complex problem to reason about')
    .option('--steps', 'Show step-by-step reasoning')
    .description('Use DSC2 for complex reasoning and planning')
    .action(async (prompt, options) => {
      await codeReason({ prompt, steps: options.steps });
    });
}

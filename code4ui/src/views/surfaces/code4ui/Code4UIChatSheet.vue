<template>
  <aside class="code4ui-chat" :class="{ open: store.chatOpen }">
    <!-- Chat Header -->
    <div class="chat-header">
      ...
      <button class="chat-close-btn" @click="store.toggleChat">
        <span class="material-symbols-outlined chat-close-icon">close</span>
      </button>
    </div>
    ...
    <div class="chat-messages" ref="messagesContainer">
      <div v-if="store.chatMessages.length === 0" class="chat-empty">
        <span class="material-symbols-outlined chat-empty-icon">smart_toy</span>
      ...
      >
        <div class="chat-message-avatar">
          <span class="material-symbols-outlined chat-avatar-icon">{{ msg.role === 'user' ? 'person' : 'smart_toy' }}</span>
        </div>
    ...
    <div class="chat-input-area">
      <div class="chat-input-wrapper">
        ...
        <button class="chat-send-btn" @click="sendMessage" :disabled="!inputText.trim()">
          <span class="material-symbols-outlined chat-send-icon">send</span>
        </button>
      </div>
    </div>
  </aside>
...
.code4ui-chat {
  ...
  border-left: 1px dashed var(--usx-color-outline);
  display: flex;
...
.code4ui-chat.open {
  width: var(--code4ui-chat-width);
  min-width: var(--code4ui-chat-width);
}
...
.chat-avatar-icon,
.chat-send-icon {
  font-size: calc(var(--code4ui-font-size) * 1.1) !important;
}
...
.chat-empty {
  ...
  text-align: center;
  color: var(--usx-color-on-surface-variant);
  font-size: calc(var(--code4ui-font-size) * 0.85);
  padding: 24px;
  gap: 12px;
}
...
.chat-input-area {
  padding: 12px 16px;
  border-top: 1px dashed var(--usx-color-outline);
  flex-shrink: 0;
}

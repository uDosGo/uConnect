<template>
  <aside class="code4ui-nav-rail" :class="{ collapsed: store.sidebarCollapsed }">
    <!-- Top Actions -->
...
<style scoped>
.code4ui-nav-rail {
  width: var(--code4ui-sidebar-width);
  min-width: var(--code4ui-sidebar-width);
  background: var(--usx-color-surface-variant);
  display: flex;
  flex-direction: column;
  border-right: 1px dashed var(--usx-color-outline);
  transition: width 0.2s, min-width 0.2s;
  overflow: hidden;
  font-size: var(--code4ui-font-size);
}

.code4ui-nav-rail.collapsed {
  width: var(--code4ui-sidebar-collapsed-width);
  min-width: var(--code4ui-sidebar-collapsed-width);
}
...
.nav-rail-btn .material-symbols-outlined {
...
.nav-rail-tab:hover {
  background: var(--usx-color-surface-container);
  color: var(--usx-color-on-surface);
  border-color: var(--usx-color-outline);
}

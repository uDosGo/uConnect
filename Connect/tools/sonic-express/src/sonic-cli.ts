...
export class SonicCLI {
  ...
  private setupAdvancedCommands(): void {
    ...
    this.program
      ...
      .option("--game <game>", "Check specific game container health")
      .option("--all", "Check all game containers health")
      .action(async (options) => {
        if (options.game) {
          console.log(chalk.blue(`🩺 Checking health for game: ${options.game}`));
          // In a real implementation, this would call Sonic-Screwdriver's health command
          const result = this.runSonicCommand(["health", options.game]);
          console.log(result);
        } else if (options.all) {
          console.log(chalk.blue("🩺 Checking health for all game containers"));
    ...
    this.program
      ...
      .option("--game <game>", "Repair specific game container")
      .option("--all", "Repair all unhealthy containers")
      .action(async (options) => {
        if (options.game) {
          console.log(chalk.blue(`🩹 Repairing game container: ${options.game}`));
    ...
    this.program
      .command("container:start <game>")
      .description("Start a game container")
      .action(async (game) => {
        console.log(chalk.blue(`🚀 Starting container for game: ${game}`));
        const result = this.runSonicCommand(["start", game]);
        console.log(result);
      });

    this.program
      .command("container:stop <game>")
      .description("Stop a game container")
      .action(async (game) => {
        console.log(chalk.blue(`🛑 Stopping container for game: ${game}`));
        const result = this.runSonicCommand(["stop", game]);
        console.log(result);
      });

    this.program
      .command("container:logs <game>")
      .description("Show container logs")
      .option("--follow", "Follow log output")
      .option("--lines <count>", "Number of lines to show", "100")
      .action(async (game, options) => {
        console.log(chalk.blue(`📜 Showing logs for game: ${game}`));

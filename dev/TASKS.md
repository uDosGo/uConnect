...
> **📋 CONSOLIDATED RELEASE SCHEDULE:** [`~/Code/DevStudio/RELEASE_SCHEDULE.md`](../~/Code/DevStudio/RELEASE_SCHEDULE.md)  
> For release priorities, version ladder, plans vs actuals, and dev workflow overview, see the consolidated schedule.
...
| **T-UCODE1-SPRINT1**         | uCode1 Sprint 1: Core Interpreter Completion                           | **Done (2026-05-05).** All major interpreter features implemented: WHILE/ENDWHILE, CASE/OF/OTHERWISE/ENDCASE, ON ERROR/RESUME, PROC/FN with parameters, DATA/READ/RESTORE, RND with seeding, PROC_*/FN_* extension dispatch for LENS/SKIN/MCP/Spool. See `dev/HANDOVER.md` and `dev/devlog/2026-05-05-ucode1-sprint1-complete.md`. |
| **T-UCODE1-SPRINT2**         | uCode1 Sprint 2: LENS/SKIN/MCP/Spool Polish                           | **Next.** Implement LENS memory breakpoints, SKIN CeefaxThinUI bridge, MCP gRPC server, Spool compression and cross-snack import/export. See `uCode1/SPRINT_TODO.md`. |
| **T-UCODE1-SPRINT3**         | uCode1 Sprint 3: Snack Container CLI                                   | **Queued.** Add snack create/run/stop/list/pack/install commands, snackpack run, mcp send, spool commands. See `uCode1/SPRINT_TODO.md`. |
| **T-UCODE1-SPRINT4**         | uCode1 Sprint 4: NetHack & Eamon Integration                           | **Queued.** Create snack manifests, boot scripts, teletext renderer bridge, game-specific LENS rules, classic_adventures snackpack. See `uCode1/SPRINT_TODO.md`. |
| **T-UCODE1-SPRINT5**         | uCode1 Sprint 5: Documentation & Release                               | **Queued.** CLI docs, BBC BASIC extension docs, quick-start tutorial, snack/snackpack guide, pip package, version management, release checklist. See `uCode1/SPRINT_TODO.md`. |

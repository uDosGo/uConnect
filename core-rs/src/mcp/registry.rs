...
pub fn default_tools() -> Vec<ToolSpec> {
    vec![
        ...
        tool("feed_vector_search", "Search feeds using vector similarity", "A2-stub"),
        tool("feed_hybrid_search", "Search feeds using hybrid keyword+vector", "A2-stub"),
        tool("vector_index_status", "Get vector index status", "A2-stub"),
        tool("feed_vectorize", "Generate embeddings for feed items", "A2-stub"),
...
pub fn handle_tool_call(method: &str, params: Value) -> Result<Value> {
    match method {
        ...
        // Vector search tools (will use mock implementation until sqlite-vec is available)
        "feed_vector_search" => crate::mcp::vector::tools::VectorSearchTools::new().feed_vector_search(params),
        "feed_hybrid_search" => crate::mcp::vector::tools::VectorSearchTools::new().feed_hybrid_search(params),
        "vector_index_status" => crate::mcp::vector::tools::VectorSearchTools::new().vector_index_status(params),
        "feed_vectorize" => crate::mcp::vector::tools::VectorSearchTools::new().vectorize_feed_items(params),
        _ => Ok(json!({
            "ok": false,
